...
impl Job {
    ...
    pub fn is_fully_contained_in(&self, other: &Job) -> bool {
        self.min_id >= other.min_id && self.max_id <= other.max_id
    }
}
...
impl PairAssignment {
    ...
    pub fn is_one_of_pair_fully_contained_in_the_other(&self) -> bool {
        self.job1.is_fully_contained_in(&self.job2) || self.job2.is_fully_contained_in(&self.job1)
    }
...
fn get_pair_assignments_from_input(input: &str) -> Vec<PairAssignment> {
    input
        ...
        .collect::<Vec<&str>>()
        .iter()
        .map(|x| PairAssignment::from_string(x))
...
pub fn get_total_number_of_fully_contained_assignment_pairs(input: &str) -> u32 {
    ...
    pair_assignments
        .into_iter()
        .filter(|pair| pair.is_one_of_pair_fully_contained_in_the_other())
        .collect::<Vec<PairAssignment>>()
        .len() as u32
}
...
mod tests {
    ...
    fn test_is_one_of_pair_fully_contained_in_the_other(
        #[case] j1: Job,
        #[case] j2: Job,
        #[case] expected: bool,
    ) {
        ...
        assert_eq!(
            pair_assignment.is_one_of_pair_fully_contained_in_the_other(),
            expected
        )
    }
